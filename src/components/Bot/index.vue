<template>
    <transition name="fade-bounce-y" mode="out-in">
        <div v-show="openAiBot" id="bot-container">
            <div id="Aurora-Dia--body" :style="cssVariables" @click="toggleChat">
            ...
            <chat :visible="showChat" @close="showChat = false" />
        </div>
    </transition>
</template>
...
const initializeBot = () => {
    if (!openAiBot.value) return
    const dia = new AuroraDia()
    dia.installSoftware({
        locale: locale.value === 'zh' ? 'cn' : locale.value
    })
    dia.on()
}

watch([openAiBot, locale], () => {
    initializeBot()
})
